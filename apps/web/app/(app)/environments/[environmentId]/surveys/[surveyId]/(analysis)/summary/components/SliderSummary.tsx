...
import { getDecimalScale } from "@formbricks/types/surveys/constants";
...
// `toFixed` is specified for 0 to 100 fraction digits and throws a RangeError outside that, while a step may
// legitimately be as fine as MAX_GRID_DECIMAL_SCALE (300) decimal places. Clamping here keeps a lawful
// configuration from crashing the card; a mean needing more than 100 places is displayed truncated rather
// than not at all.
const MAX_DISPLAY_DECIMALS = 100;

// Fewest decimals any average is shown with. Two is what every summary card in this folder uses, so an
// integer-stepped Slider - every preset and the overwhelming majority of real configurations - reads exactly
// as its siblings do.
...
export const SliderSummary = ({ elementSummary, survey }: SliderSummaryProps) => {
  ...
  // conventionally shows and capped just past the configured step's scale: the step is the finest distinction
  // the author asked for, and two extra places cover a mean landing between two grid points. An
  // integer-stepped Slider therefore yields exactly two decimals, identical to every sibling card.
  const stepScale = getDecimalScale(elementSummary.element.step) ?? 0;
  ...
  const displayDecimals = Math.min(
    ...
    stepScale + MIN_DISPLAY_DECIMALS,
